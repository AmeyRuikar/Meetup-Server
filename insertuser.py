...
class MainPage(webapp2.RequestHandler):
    def get(self):
        ...
        fbname = self.request.get('fbname')
        username = self.request.get('username')
        interests = self.request.get('interests')
        trans = self.request.get('trans')

        query = "insert into users(userid, fbname, username, interests, transportationPref, lastLogin) values ('%s','%s', '%s', ('%s'), '%s', %d)"%(userid, fbname, username, interests, trans, int(time.time()))
